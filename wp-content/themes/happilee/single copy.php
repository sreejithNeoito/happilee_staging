		...
		?>
			...
			<div class="container flex justify-between lgmd:flex-col">
				<div class="content-left w-8/12 lgmd:w-full p-5 pr-8 lgmd:pr-5">
					<h1 class="title leading-[44px] text-40 font-semibold text-primary mb-6"><?php echo get_the_title(); ?></h1>
				...
				<div class="content-right w-4/12 lgmd:w-full p-5 pl-8 lgmd:pl-5 flex flex-col gap-6 sticky top-[75px] h-max">
					<div class="rounded-[10px] text-24 w-full leading-[26px] text-primary">Table of<b> Contents</b></div>
					<div class="flex flex-col border-0 border-b border-[#0B39661A] pb-6 gap-2 lgm:max-h-[288px] lgm:overflow-y-scroll lgmd:h-full toc-container happilee-scroll">
						<?php
						...
						if (!empty($matches[0])):
							foreach ($matches[2] as $index => $heading):
								...
								<div class="flex gap-2 items-center toc-item">
					...
					<?php echo do_shortcode('[forminator_form id="1048"]') ?>
				</div>
			</div>
		<?php
		endwhile;
